...
export function CatalogTab({ lang, telegramId, favIds, onToggleFav }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { const data = await getProducts({ category, filters }); setProducts(data) }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <CategoryBar active={category} lang={lang} onFiltersClick={() => setShowFilters(true)} onCategoryChange={(cat) => setCategory(cat)} />

      <div className="flex-1 overflow-y-auto p-3">
        {loading ? (
          <div className="grid grid-cols-2 gap-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="rounded-2xl bg-bg-2 animate-pulse aspect-[3/4]" />
            ))}
          </div>
        ) : products.length === 0 ? (
          <div className="flex items-center justify-center h-40">
            <p className="text-text-muted text-sm uppercase tracking-widest">{t(lang, 'emptyCatalog')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            {products.map((p) => (
